  ...
  read.interval = setInterval(function() {
    ...
    if (boards.length && reporting.length) {
      ...
      reporting.forEach(function(report, gpio) {
        board.pins[report.index].read(function(error, value) {
    ...
    mode: {
      ...
      set: function(value) {
        if (value === 0 || value === 2) {
          ...
          // TODO: Make sure this doesn't actually
...
Pin.prototype.read = function(callback) {
  ...
  if (state.isAnalog) {
    state.pin.analogRead(callback);
  } else {
    state.pin.read(callback);
  }
};
