...
Tessel.prototype.serialConfig = function(options) {
  ...
  this.serialPorts[options.bus] = new port.UART({ "baudrate": baud, "dataBits": dataBits, "parity": parity, "stopBits": stopBits });

};

Tessel.prototype.serialWrite = function(portId, inBytes) {
  if (this.serialPorts[options.bus]) {
    this.serialPorts[options.bus].write(new Buffer(inBytes));
  }
};

Tessel.prototype.serialRead = function(portId, maxBytesToRead, callback) {
  if (this.serialPorts[options.bus]) {
    this.serialPorts[options.bus].on("data", callback);
  }
};

Tessel.prototype.serialStop = function(portId) {
  if (this.serialPorts[options.bus]) {
    this.serialPorts[options.bus].removeAllListeners("data");
  }
};
