...
function Tessel(options) {
  ...
  // Used to store serialport objects?
  this.serialPorts = {};
...
Tessel.prototype.serialConfig = function(options) {
  ...
  // Default to Port A's UART (user facing name is "A")
  if (!options.bus) {
    // Maybe they called it "port"? Let's allow that.
    if (options.port) {
      options.bus = options.port;
  ...
  var port = tessel.port[options.bus];
  state.serialPorts[options.bus] = new port.UART({ "baudrate": baud, "dataBits": dataBits, "parity": parity, "stopBits": stopBits });
};
...
Tessel.prototype.serialWrite = function(portId, inBytes) {
  ...
  if (state.serialPorts[options.bus]) {
    state.serialPorts[options.bus].write(new Buffer(inBytes));
  }
};

Tessel.prototype.serialRead = function(portId, maxBytesToRead, callback) {
  var state = priv.get(this);
  if (state.serialPorts[portId]) {
    state.serialPorts[portId].on("data", callback);
  }
};
...
Tessel.prototype.serialStop = function(portId) {
  var state = priv.get(this);
  if (state.serialPorts[portId]) {
    state.serialPorts[portId].removeAllListeners("data");
  }
};
