...
Tessel.prototype.digitalRead = function(pin, callback) {
  ...
  var identity = ToPortIdentity(pinIndex);
  var event = "digital-read-" + pinIndex;

  // The Firmata protocol only _emits_ new digital
  // read data when that data is different from the
  ...
  handler.isDigitalReadFilter = true;

  this.on(event, callback);


  // If the pin has interrupt capabilities, and
  // there are currently no interrupts registered
  // for this pin's port, then use the interrupt.
  if (this.pins[pinIndex].isInterrupt &&
      state.interrupts[identity.port] === null) {
    state.interrupts[identity.port] = identity.index;
