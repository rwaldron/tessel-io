...
function adcReadRequestLoop() {
  adcReadRequestLoop.isActive = true;

  /* istanbul ignore else */
  if (adcReadRequests.length) {
    // Shift the next request object from the beginning of the list.
    const read = adcReadRequests.shift();

    read.port.sock.write(read.command);
    read.port.enqueue({
      size: read.size,
      callback(error, data) {
        /* istanbul ignore if */
        ...
        read.callback(data.readUInt16LE(0) >> 2);

        // Put the request object at the end of the list.
        adcReadRequests.push(read);
...
adcReadRequestLoop.isActive = false;
...
class Pin extends Emitter {
  ...
  read(callback) {
    ...
    // Handle ADC/ANALOG_READ requests separately
    ...
    if (state.commands.read === null) {
      state.commands.read = new Buffer([Port.CMD.GPIO_IN, index]);
    }
    ...
    const poll = _ => {
      ...
      state.pin.port.enqueue({
        ...
        callback(error, data) {
          ...
          callback(data === Port.REPLY.HIGH ? 1 : 0);
